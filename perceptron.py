...
class PerceptronModel:
    ...
    def score(self, datapoint: DataPointWithFeatures, label: str) -> float:
        ...
        """
        return sum(
            datapoint.features.get(f, 0.0)
            * self.weights.get(self._get_weight_key(f, label), 0.0)
            for f in datapoint.features
        )
    ...
    def update_parameters(
        ...
        Inputs:
            ...
            lr: Learning rate.
        """
        correct = datapoint.label
        for f, v in datapoint.features.items():
            self.weights[self._get_weight_key(f, correct)] += lr * v
            self.weights[self._get_weight_key(f, prediction)] -= lr * v
    ...
    def evaluate(
        ...
        if all(dp.label is not None for dp in data):
            return accuracy(preds, [dp.label for dp in data])
        return 0.0
